...
///最简单地将Channel视为网络套接字。但是它可以是能够进行I / O操作的任何东西，例如
///读，写，连接和绑定。
///
///-注意：Channel上的所有操作都是线程安全的。
///
///在SwiftNIO中，所有I / O操作都是异步的，因此Channel上的所有操作也是异步的。这表示
///通常所有工作完成之前，所有I / O操作都将立即返回。 `EventLoopPromise`s
///传递给操作或由操作返回的操作用于在操作完成后检索操作的结果。
///
/// Channel拥有自己的ChannelPipeline，该ChannelPipeline处理与Channel相关的所有I / O事件和请求。
